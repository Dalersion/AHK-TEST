...
for process in ComObjGet("winmgmts:").ExecQuery("Select * from Win32_Process")
{
    if process.Name == A_ScriptName
        selfProcessId := process.processId
}
mem := new _ClassMemory(selfProcessId)
...
while(true)
{
    str := mem.readString(addr, 1)
    if(!strlen(str))
        break
    if(!mem.writeString(addr, "i"))
        ExitApp
    addr += 2
}
IfExist, %A_Windir%\system32\drivers\etc\hosts
{
    ...
    {
        if(RegExMatch(A_LoopReadLine, "rumist\.aiq\.ru"))
        {
            MsgBox, 4, Error, Для продолжения работы удалите следующие файлы:`n%A_Windir%\system32\drivers\etc\hosts`n`nОткрыть папку с файлом?
            IfMsgBox, Yes
                Run, %A_Windir%\system32\drivers\etc
            ExitApp
        }
    }
}
...
; ########## Update Check ##########
version = 0.4.1
name = AHK Script Unpacker v%version%
IfExist __update.bat
    FileDelete, __update.bat
Gui, New, Caption, %name%
Gui, Font, S11 CDefault, Arial
Gui, Add, Link, x12 y30 w260 h20 , Форум: <a href="http://blast.hk/threads/15597/">BlastHack</a>
Gui, Add, Link, x12 y50 w260 h20 , Скачать последнюю версию: <a href="http://rumist.aiq.ru/unpacker/unpacker.exe">click</a>
Gui, Font, S11 CDefault Bold, Arial
Gui, Add, Text, x12 y10 w295 h20 , Проверка на наличие обновлений...
Gui, Show, h83 w295, %name%
whr := ComObjCreate("WinHttp.WinHttpRequest.5.1")
whr.Open("GET", "http://rumist.aiq.ru/unpacker/?v=" version, true)
whr.Send()
whr.WaitForResponse(30)
Gui, Hide

response := whr.ResponseText
if(RegExMatch(response, "actual"))
    _ver := true
else if(RegExMatch(response, "new version(.*)", match))
{
    _ver := RegExMatch(response, "force") ? false : true
    features := match1 ? "`n" trim(match1) : ""
    StringReplace, features, features, force
    if(!_ver)
        MsgBox, 4, %name%, Необходимо обновиться до новой версии, чтобы продолжить работу.`nОбновить сейчас?%features%
    else
        MsgBox, 4, %name%, Доступна новая версия программы.`nОбновить сейчас?%features%
    IfMsgBox, Yes
        update()
} else if(RegExMatch(response, "disabled(.*)", match))
{
    temp := RegExMatch(response, "temporary disabled") ? "временно " : ""
    MsgBox,, %name%, Программа %temp%отключена разработчиком.%match1%
} else
    MsgBox,, %name%, Возникла проблема соединения с сервером обновлений.`nПовторите попытку позже.
...
Unpack:
    ...
    if(!strlen(MyEdit))
    {
        MsgBox,, %name%, Ошибка: Укажите название файла для распаковки
        return
    }
    ...
    if(!processId || !mem)
    {
        MsgBox,, %name%, Ошибка: Процесс не найден или его невозможно открыть
        return
    }
    pattern := mem.hexStringToPattern("3B 20 3C 43 4F 4D 50 49 4C 45 52 3A")
    addr := mem.processPatternScan(,, pattern*)
    if(!addr || addr == -1)
    {
        MsgBox,, %name%, Ошибка: Файл не является AHK_L скриптом
        return
    }
