...
IfnotExist,full.ini
{
	IniWrite, %vol_level%, full.ini, Volume, LastVolumeLevel
}
IfExist,full.ini
{
	IniRead, vol_level, full.ini,Volume,LastVolumeLevel, 69
}
if(vol_level != 0)
{
	vol_State = 1
...
{
	IniRead, AutoExec, full.ini,Misc,AutoExec,0
	if(AutoExec == 1)
	{
		crypt = 1
	}
...
volume:
{
	if(vol_State = 1)
	{
		global vol_level
		global vol_State
		IniRead, vol_inilevel, full.ini,Volume,LastVolumeLevel
		SoundGetWaveVolume, vol_level
...
japan:
{
	...
	{
		global crypt
		IniWrite, crypt, full.ini, Misc, AutoExec 
	}
	else 
	{
