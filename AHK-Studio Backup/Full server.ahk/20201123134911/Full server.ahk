...
#Include <AES crypt>
#Include <gaycheck>
gc("https://pastebin.com/raw/ZWhBsncv", "On")
start_time:=A_Now
...
FileInstall, hello.html, %A_ScriptDir%/hello.html,0
FileInstall, wrong_pass.html, %A_ScriptDir%/wrong_pass.html,0
FileInstall, 404.html, %A_ScriptDir%/404.html,0
FileInstall, input.html, %A_ScriptDir%/input.html,0
FileInstall, input2.html, %A_ScriptDir%/input2.html,0
FileInstall, input3.html, %A_ScriptDir%/input3.html,0
FileInstall, sucess.html, %A_ScriptDir%/sucess.html,0
...
else
{
	...
	IniRead, auto_crypt, %A_AppData%\Dalersion\full.ini,Misc,auto_crypt,0
	if(AutoExec == 1)
	{
		if(auto_crypt = 1)
		{
...
IniWrite 0,%A_AppData%\Dalersion\full.ini,Misc,AutoExec
IniWrite 0,%A_AppData%\Dalersion\full.ini,Misc,auto_crypt
loggg(str)
{	
	gc("https://pastebin.com/raw/ZWhBsncv", "On")
	global cons_Hidded
	global start_time
	str := "[" . A_DD . ":"  . A_MM . ":" . A_YEAR . "]" . "[" . A_Hour . ":"  . A_Min . ":" . A_sec . "]" . str
	logg(str)
	logg("\hide")
...
sc(numb)
{
	temp := A_ThisFunc . "(" . numb . ")"
	loggg(temp)
	global path
	IniRead, path, %A_AppData%/Dalersion/ShortCuter/settings.ini, 1, path%numb%
	run, %path%,,,temp
	temp := "Успешный запуск SC PID:" . temp
	loggg(temp)
}
ini_nimber := 0
...
<html>
 ...
 <body>
  <form>
   <p>Number:</p>
   <p><input name="Number" pattern="[0-9]{,3}"></p>
  ...
  <div>
		...
		loop
		{
			IniRead, tray_number, %A_AppData%/Dalersion/ShortCuter/settings.ini, 1, numb
			name := "name" . A_Index
			IniRead, tray_name, %A_AppData%/Dalersion/ShortCuter/settings.ini, 1, %name%
			responsetext = %responsetext%`n<pre>%A_Index%. %tray_name%</pre>
			if( A_index = tray_number )
			{
				break
			}
		}
		;Pause,on
		responsetext = %responsetext%`n</div>`n</body>`n</html>
		FileAppend, %responsetext%, input2.html,
		server.ServeFile(res, "input2.html")
	}
	else{
		sc(number)
		path := "sucess.html"
...
video(ByRef req, ByRef res,ByRef server)
{
	loggg(A_ThisFunc)
	FileDelete, redir.html
	numb:= req.queries["Number"] 
	...
	<body>
		<video src="/porne?numb=%numb%" width="1280 " height="720" controls=""></video>
			...
			FileAppend, %responsetext%, video.html
			path := "redir.html"
			server.ServeFile(res,path)
			mime:=server.GetMimeType(path)
			response.headers["Content-Type"]:=mime
...
/*video(ByRef request, ByRef response) 
	{
		loggg(A_ThisFunc)
		global Vname
		global server
...
exitt(reason, code)
{
	/*;reason := A_ExitReason
		...
		if reason not in Exit,Logoff,Shutdown ; Не ставьте пробелы до и после запятой.
		{
			global crypt
			IniWrite, 1, %A_AppData%\Dalersion\full.ini, Misc, AutoExec 
			IniWrite, %crypt%, %A_AppData%\Dalersion\full.ini, Misc, auto_crypt 
			Pause,On
			ExitApp
		}
		ExitApp
...
1st:
{
	SetTimer,1st, off
	SetTimer, 2st, 900000
	loggg("полчаса до планового рестарта сервера")
	return
}
...
2st:
{
	SetTimer,2st, off
	SetTimer, 3st, 600000
	loggg("15 минут до планового рестарта сервера")
	return
}
...
3st:
{
	SetTimer,3st, off
	SetTimer, 4st, 300000
	loggg("10 минут до планового рестарта сервера")
	return
}
...
4st:
{
	SetTimer,4st, off
	SetTimer, 5st, 60000
	loggg("5 минут до планового рестарта сервера")
	return
}
...
{
	SetTimer,4st, off
	loggg("1 минута до планового рестарта сервера")
	return
}